...
@J2clTestInput(CharBufferTest.class)
public class CharBufferTest extends AbstractBufferTest {
  ...
  public void testSubSequence() {
    ...
    assertEquals(buf.subSequence(0, buf.length()).toString(), buf.toString());

    if (buf.length() >= 2) {
      assertEquals(
          buf.subSequence(1, buf.length() - 1).toString(),
          buf.toString().substring(1, buf.length() - 1));
    }
  }
  ...
  private void assertContentEquals(CharBuffer buf, char array[], int offset, int length) {
    for (int i = 0; i < length; i++) {
  ...
  private void assertContentLikeTestData1(
      CharBuffer buf, int startIndex, char startValue, int length) {
  ...
  public void test_WrapLjava_lang_CharSequenceII() {
    ...
    CharBuffer buf2 =
        CharBuffer.wrap(
            buf.toString()
                + buf.toString()); // buf.toString() + buf.toString()  //"123456789a123456789a"
